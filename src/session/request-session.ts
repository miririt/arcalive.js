...
abstract class RequestSession {
  ...
  async _fetch(
    ...
    init: RequestOption = { csrfRequired: false }
    ...
    if (csrfRequired) {
      ...
      else init.body = new URLSearchParams();
  ...
  async getArticle(articleId: number): Promise<Article> {
    ...
    const articleUrl = `https://arca.live/b/placeholder/${articleId}`;
    const response = await this._fetch(articleUrl);

    return this.articleFromUrl(response.url);
  }
...
class AnonymouosSession extends RequestSession {
   ...
   * @param {string} password
   */
  setAnonymous(nickname: string, password: string) {
