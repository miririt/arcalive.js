...
class Article {
  ...
  async read(
    ...
    if (options.noCache || !this._loaded || !this._articleData) {
      ...
      if (badge !== null) {
        ...
        this._articleData.title = articleTitle.innerText.replace(
          this._articleData.category,
          ""
        );
      } else {
      ...
      this._articleData.author =
        memberInfo.querySelector(".user-info").firstChild.attributes[
          "data-filter"
        ];

      this._articleData.time = new Date(
      ...
      this._articleData.rateDiff =
        this._articleData.rate[0] - this._articleData.rate[1];

      this._articleData.content = article.querySelector(
      ...
      if (options.withComments) {
        ...
        for (let i = lastCommentPage; i >= 1; i--) {
          const commentPage = await this._session._fetch(
            `${this._board.url}/${this.articleId}?cp=${i}`
          );
          const comments = commentPage.querySelectorAll(".comment-wrapper");

          this._articleData.comments.push(
            ...comments.map((comment) => {
              ...
              if (emoticonWrapper) {
                textContent =
                  ...
                  emoticonWrapper.childNodes?.find((e) => e.rawAttrs)
                    ?.attributes?.src ||
                  "";
  ...
  deleteComment(commentId) {
    const commentObject =
      this._articleData.comments[commentId] ||
      new Comment(this, {
        commentId: commentId,
      });
...
Article.ArticleData = class ArticleData {
